...
typedef struct {
	...
	volatile int counter; // Counter for how many rows have been processed.
	//char padding[64-sizeof(int)*2-sizeof(pthread_t)-sizeof(double)];
} thread_info_t;
...
thread_sweep(int tid, int iter, int lbound, int rbound)
{
		...
		if(tid != 0) {	
			dprintf("eeeh gg, previous %d, mine %d \n", threads[tid-1].counter, threads[tid].counter);		
		}
                /* TASK: Wait for data to be available from the thread
                 * to the left */
		if(tid != 0){
			while(threads[tid-1].counter <= threads[tid].counter){
				continue;	// spin	
			}
		};
...
thread_compute(void *_self)
{
	...
	lbound = (gs_size/gs_nthreads)*tid; // padding???
	rbound = lbound+(gs_size/gs_nthreads);
	if(tid == 0) lbound++;
